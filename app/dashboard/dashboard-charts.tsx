...
function pnlTooltipFormatter(value: unknown): [ReactNode, string] {
  if (value == null) return ["—", "P&L"];
  const num = typeof value === "number" ? value : Number(value);
  if (Number.isNaN(num)) return [String(value ?? "—"), "P&L"];
        ...
        <div className="min-h-0 min-w-0 flex-1">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={byUnderlyingPnl}>
              ...
              />
              <Tooltip formatter={pnlTooltipFormatter} />
              <ReferenceLine y={0} stroke="#a1a1aa" strokeDasharray="4 4" />
              <Bar dataKey="pnl" name="P&L" radius={[4, 4, 0, 0]}>
                {byUnderlyingPnl.map((row) => (
                  <Cell
                    key={row.underlying}
                    fill={
                      row.pnl < 0
